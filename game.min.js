...
a=[cc.p(c.x,c.y),cc.p(c.x+a.width,c.y),cc.p(c.x+a.width,c.y+a.height),cc.p(c.x,c.y+a.height)];cc._drawingUtil.setDrawColor(0,255,0,255);cc._drawingUtil.drawPoly(a,4,!0)}};a._updateCharColorAndOpacity=function(){}})();cc._globalFontSize=cc.ITEM_SIZE;cc._globalFontName="Arial";cc._globalFontNameRelease=!1;
...
c.length)){this.steps++;this.moveing=!0;for(d=0;d<c.length;d++)f=c[d],this.mapPanel.removeChild(f),this.map[f.column][f.row]=null;this.score+=c.length*c.length;this._generateNewCandy();this._checkSucceedOrFail()}}},_checkCandyExist:function(a,b){return 0<=a&&a<Constant.MAP_SIZE&&0<=b&&b<Constant.MAP_SIZE?!0:!1},_generateNewCandy:function(){for(var a=0;a<Constant.MAP_SIZE;a++){for(var b=0;b<this.map[a].length;b++){var c=this.map[a][b];c||(c=Candy.createRandomType(a,Constant.MAP_SIZE+0),this.mapPanel.addChild(c),
c.x=c.column*Constant.CANDY_WIDTH+Constant.CANDY_WIDTH/2,c.y=c.row*Constant.CANDY_WIDTH+Constant.CANDY_WIDTH/2,this.map[a][c.row]=c)}for(b=this.map[a].length;b>=Constant.MAP_SIZE;b--)this.map[a].splice(b)}this.scheduleOnce(this._finishCandyFalls.bind(this),0)},_checkSucceedOrFail:function(){this.score>this.targetScore?(this.ui.showSuccess(),this.score+=30*(this.limitStep-this.steps),this.scheduleOnce(function(){cc.director.runScene(new GameScene)},3),Storage.setCurrentLevel(this.level+1),Storage.setCurrentScore(this.score)):
this.steps>=this.limitStep&&(this.ui.showFail(),this.scheduleOnce(function(){cc.director.runScene(new GameScene)},3),Storage.setCurrentLevel(0),Storage.setCurrentScore(0))},_finishCandyFalls:function(){this.moveing=!1}}),GameScene=cc.Scene.extend({onEnter:function(){this._super();var a=new GameLayer;this.addChild(a)}});var SecondScene=cc.Scene.extend({onEnter:function(){this._super();var a=new cc.LayerGradient(cc.color(255,0,0),cc.color(0,0,255));this.addChild(a);setTimeout(function(){cc.director.runScene(new cc.TransitionSlideInT(2,new HelloWorldScene))},300)}}),BallLayer=cc.Layer.extend({deltax:1,boll:null,frame:0,bg:null,ctor:function(){this._super();var a=cc.director.getWinSize(),b=new cc.Sprite(res.item2_png);b.x=0;b.y=a.height/2;this.addChild(b,1);this.ball=b;this.bg=new cc.DrawNode;this.addChild(this.bg);
this.scheduleUpdate();return!0},update:function(){var a=cc.director.getWinSize();this.ball.x+=this.deltax;if(this.ball.x>a.width||0>this.ball.x)this.deltax*=-1;this.ball.y=50*Math.sin(this.frame/20)+a.height/2;this.bg.drawDot(new cc.Point(this.ball.x,this.ball.y),2,cc.color(225,0,0));this.frame++}}),SimplyActionLayer=cc.Layer.extend({ctor:function(){this._super();var a=cc.director.getWinSize(),b=new cc.Sprite(res.item2_png);b.x=a.width/2;b.y=a.height;this.addChild(b,1);a=cc.moveBy(2,0,-(a.height-
b.height/2));a.easing(cc.easeIn(2));var c=a.clone().reverse();c.easing(cc.easeBounceIn());b.runAction(cc.sequence(a,c));return!0}});var Candy=cc.Sprite.extend({type:0,column:0,row:0,ctor:function(a,b,c){this._super("res/"+(a+1)+".png");this.init(a,b,c)},init:function(a,b,c){this.type=a;this.column=b;this.row=c}});Candy.createRandomType=function(a,b){return new Candy(parseInt(Math.random()*Constant.CANDY_TYPE_COUNT),a,b)};var Constant={CANDY_TYPE_COUNT:5,CANDY_WIDTH:64,MAP_SIZE:10,FALL_ACCELERATION:10,levels:[{limitStep:30,targetScore:500},{limitStep:25,targetScore:1E3},{limitStep:20,targetScore:2E3},{limitStep:20,targetScore:3E3},{limitStep:15,targetScore:4E3},{limitStep:10,targetScore:5E3}]};var GameUI=cc.Layer.extend({levelText:null,scoreText:null,stepText:null,gameLayer:null,ctor:function(a){this._super();this.gameLayer=a;this._initInfoPanel();this.scheduleUpdate()},_initInfoPanel:function(){var a=cc.director.getWinSize(),b=new cc.LabelTTF("Level","arial",36);b.x=100;b.y=a.height-50;b.setColor(cc.color(0,0,0));this.addChild(b);a=new cc.LabelTTF("1","arial",36);a.x=100;a.y=b.y-40;a.setColor(cc.color(0,0,0));this.addChild(a);this.levelText=a;var c=new cc.LabelTTF("Score","arial",36);
c.x=370;c.y=b.y;c.setColor(cc.color(0,0,0));this.addChild(c);c=new cc.LabelTTF("0","arial",36);c.x=370;c.y=a.y;c.setColor(cc.color(0,0,0));this.addChild(c);this.scoreText=c;c=new cc.LabelTTF("Step","arial",36);c.x=620;c.y=b.y;c.setColor(cc.color(0,0,0));this.addChild(c);b=new cc.LabelTTF("0","arial",36);b.x=620;b.y=a.y;b.setColor(cc.color(0,0,0));this.addChild(b);this.stepText=b},update:function(){this.levelText.setString(""+(this.gameLayer.level+1));this.scoreText.setString(""+this.gameLayer.score);
this.stepText.setString(""+(this.gameLayer.limitStep-this.gameLayer.steps))},showSuccess:function(){var a=new cc.LayerColor(cc.color(255,255,255),500,500);this.addChild(a,1);var b=cc.director.getWinSize();a.x=(b.width-a.width)/2;a.y=(b.height-a.height)/2;b=new cc.LabelTTF("\u606d\u559c\uff0c\u5df2\u7ecf\u5b8c\u6210\u7b2c"+(this.gameLayer.level+1)+"\u5173\uff0c\n\u5269\u4f59\u6b65\u9aa430\u500d\u5956\u52b1");b.setColor(cc.color(0,0,0));b.x=250;b.y=250;a.addChild(b)},showFail:function(){var a=new cc.LayerColor(cc.color(255,
255,255),500,500);this.addChild(a,1);var b=cc.director.getWinSize();a.x=(b.width-a.width)/2;a.y=(b.height-a.height)/2;b=new cc.LabelTTF("\u5931\u8d25\u4e86\uff0c\n\u4ece\u5934\u6765\u8fc7\u5427\uff01");b.setColor(cc.color(0,0,0));b.x=250;b.y=250;a.addChild(b)}});var Storage={getCurrentLevel:function(){var a=cc.sys.localStorage.getItem("level")||0;return parseInt(a)},setCurrentLevel:function(a){cc.sys.localStorage.setItem("level",a);return!0},getCurrentScore:function(){var a=cc.sys.localStorage.getItem("score")||0;return parseInt(a)},setCurrentScore:function(a){cc.sys.localStorage.setItem("score",a);return!0}};cc.game.onStart=function(){!cc.sys.isNative&&document.getElementById("cocosLoading")&&document.body.removeChild(document.getElementById("cocosLoading"));cc.view.enableRetina(!1);cc.view.adjustViewPort(!0);cc.view.setDesignResolutionSize(720,1280,cc.ResolutionPolicy.SHOW_ALL);cc.view.resizeWithBrowserSize(!0);cc.LoaderScene.preload(g_resources,function(){cc.director.runScene(new GameScene)},this)};cc.game.run();